...
#画像、pluginデータパス。
#souceforgeだとcgi-binに画像を置けないので。do_endの処理がめんどくなりそうだけど未修正。
#script.jsにもあります。
DIR_DATA_PATH = 'http://wolften.sourceforge.jp/data/'
#トップ絵URL
TOP_BANNER = 'http://wolften.sourceforge.jp/data/img/ten_banner.jpg'
...
<link rel="stylesheet" type="text/css" href="#{DIR_DATA_PATH}plugin/base.css">
<script type="text/javascript" src="#{DIR_DATA_PATH}plugin/jquery.js"> </script>
<script type="text/javascript" src="#{DIR_DATA_PATH}plugin/script.js"> </script>
END
...
class String
	...
	def url!
		self.gsub!(/(https?\:\/\/[\w\.\~\-\?\&\+\=\:\@\%\;\#\%]+)([\w\.\~\/\-\?\&\+\=\:\@\%\;\#\%]+)/){
			dol1 = $1
			dol2 = $2
			if /http:\/\/wolften.sakura.ne.j/ =~ dol1
				dol1+dol2
			else
				'<a href="'+dol1+dol2+'" target="_blank" style="color:black;font-size:12pt;">'+dol1+'...</a>'
			end
		}
	end

	def vil_url!
		self.sub!(/\[(\d{1,5})村?\]|(http:\/\/wolften\.sakura\.ne.jp\/index.cgi\?vid=)(\d{1,5})[0-9A-Za-z_=#(&amp;)]*/){
			vil_id = ($1) ? $1 : $3
			%Q(<a href="http://wolften.sakura.ne.jp/index.cgi?vid=#{vil_id}#form" class="say" target="_blank">[人狼天国 #{vil_id}村]</a>)
		}
	end
end
