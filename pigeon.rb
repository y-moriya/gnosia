...
class Pigeon
	...
	def handle_vid
		...
		if (date == 0)
			...
			s += @vil.display_skill if (@vil.state != 0)
	...
	def handle_entry
		...
		@vildb.transaction do
			...
			if (@userid != MASTER)
				player = Player.new(pid, @userid, vil.num_id, skill, name, num_char)
				vil.num_id += 1
			else
				vil.dummy = true
			...
			s += vil.say(type, cnt, player, msg, player.userid, @cgi.remote_addr, "M")
			vil.addlog(s)
		end
		@f_success = true
	end
			...
			if (@login)
				...
				if (!guest)
					...
					if (type == 'action')
						...
						s = vil.say_action(player, msg, @userid, @cgi.remote_addr, "M")
						vil.addlog(s)
						@f_success = true
					else
						vil.say_cnt[type] = vil.say_cnt[type] + 1
						cnt = vil.say_cnt[type]
						s = vil.say(type, cnt, player, msg, @userid, @cgi.remote_addr, "M")
						vil.addlog(s)
						@f_success = true
					end
				else
					...
					vil.say_cnt[type] = vil.say_cnt[type] + 1
					cnt = vil.say_cnt[type]
					s = vil.say(type, cnt, nil, msg, @userid, @cgi.remote_addr, "M")
					vil.addlog(s)
					@f_success = true
				end
			end
	...
	def form_act
		...
		if (!@player)
			if (@vil.state == 0)
				...
				if (@vil.char == 10)
					s += %Q(名前入力：<input name="name" size="20"><br>)
				end
			...
			if (@player.dead == 0 && @vil.state == 1)
				...
				s += skill_post
					...
					if (@player.target == -1)
						s += "<option value = -1>未設定 *"
						s += "\n"
						s += "<option value = #{@player.num_id}>スウィッチを押す"
					else
						s += "<option value = #{@player.num_id}>スウィッチを押す *"
						s += "\n"
						s += "<option value = -1>キャンセル"
